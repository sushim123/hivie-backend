...
import {preFindOneAndUpdateMiddleware, preSaveMiddleware} from '../middlewares/userSchema.middleware.js';
...
const userSchema = new mongoose.Schema(
  {
    ...
    expiresAt: {
      type: Date,
      default() {
        return this.role === 'influencer' ? new Date(Date.now() + 1 * 60 * 1000) : null;
      }
    },
    platformLinks: {
      type: [platformLinksSchema],
      required() {
        return ['influencer', 'brand'].includes(this.role);
      }
    },
    pricing: {
      type: [pricingSchema],
    ...
    sizeOfCompany: {
      type: [String],
      enum: ['under 0.5 cr', '0.5-1 cr', '1-5 cr', '5-10 cr', '10-100 cr', '100+ cr'],
      required() {
        return this.role === 'brand';
      }
    },
